...
int mv2_size_iscatter_tuning_table = 0;
mv2_iscatter_tuning_table *mv2_iscatter_thresholds_table = NULL;
...
int MV2_set_iscatter_tuning_table(int heterogeneity)
{
	...
	/*Trestles Table*/
	mv2_size_iscatter_tuning_table = 5;
	mv2_iscatter_thresholds_table = MPIU_Malloc(mv2_size_iscatter_tuning_table *
						  sizeof (mv2_iscatter_tuning_table));
	mv2_iscatter_tuning_table mv2_tmp_iscatter_thresholds_table[] = {
	    {8,
	     8192, 4, 4,
	     {0},
	     1, {{0, -1, &MPIR_Iscatter_binomial, -1}},
	     1, {{0, -1, NULL, -1}}
	    },
	    {16,
	     8192, 4, 4,
      ...
      MPIU_Memcpy(mv2_iscatter_thresholds_table, mv2_tmp_iscatter_thresholds_table,
		  mv2_size_iscatter_tuning_table * sizeof (mv2_iscatter_tuning_table));
...
{
    if (mv2_iscatter_thresholds_table != NULL) {
	MPIU_Free(mv2_iscatter_thresholds_table);
    }
...
int MV2_internode_Iscatter_is_define(char *mv2_user_iscatter_inter, char *mv2_user_iscatter_intra)
{
    ...
    int i;
    int nb_element = count_sep(mv2_user_iscatter_inter) + 1;
    ...
    mv2_iscatter_tuning_table mv2_tmp_iscatter_thresholds_table[1];
    mv2_size_iscatter_tuning_table = 1;
	...
	mv2_tmp_iscatter_thresholds_table[0].is_two_level_iscatter[0] = 1;
	mv2_tmp_iscatter_thresholds_table[0].size_inter_table = 1;
	mv2_tmp_iscatter_thresholds_table[0].inter_leader[0].min = 0;
	mv2_tmp_iscatter_thresholds_table[0].inter_leader[0].max = -1;
	...
	if (mv2_user_iscatter_intra == NULL) {
	    mv2_tmp_iscatter_thresholds_table[0].intra_node[0].MV2_pt_Iscatter_function = NULL;
	} else {
	    mv2_tmp_iscatter_thresholds_table[0].intra_node[0].MV2_pt_Iscatter_function = NULL;
	}
    } else {
	...
	if (!(dup = MPIU_Strdup(mv2_user_iscatter_inter))) {
	    fprintf(stderr, "failed to duplicate `%s'\n", mv2_user_iscatter_inter);
    ...
    mv2_tmp_iscatter_thresholds_table[0].size_intra_table = 1;
    if (mv2_user_iscatter_intra == NULL) {
	mv2_tmp_iscatter_thresholds_table[0].intra_node[0].MV2_pt_Iscatter_function = NULL;
    } else {
        mv2_tmp_iscatter_thresholds_table[0].intra_node[0].MV2_pt_Iscatter_function = NULL;
    }
    MPIU_Memcpy(mv2_iscatter_thresholds_table, mv2_tmp_iscatter_thresholds_table, sizeof
...
int MV2_intranode_Iscatter_is_define(char *mv2_user_iscatter_intra)
{
    ...
    int i, j;
    for (i = 0; i < mv2_size_iscatter_tuning_table; i++) {
	for (j = 0; j < mv2_iscatter_thresholds_table[i].size_intra_table; j++) {
	    mv2_iscatter_thresholds_table[i].intra_node[j].MV2_pt_Iscatter_function = NULL;
	}
    }
    return MPI_SUCCESS;
}
