...
/*@
  MPIDI_VCRT_Get_ptr - Return a pointer to the array of VCs for this 
  reference table
  ...
  @*/
#undef FUNCNAME
#define FUNCNAME MPIDI_VCRT_Get_ptr
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
    ...
    *max_node_id = -1;
    for (i = 0; i < pg->size; i++)
        if (pg->vct[i].node_id + 1 > *max_node_id)
            *max_node_id = pg->vct[i].node_id + 1;
...
static int publish_host_id(MPIDI_PG_t *pg, int our_pg_rank)
{
    ...
    if (pg->size > 1)
    {
        ...
        pmi_errno = UPMI_KVS_COMMIT(kvs_name);
        MPIR_ERR_CHKANDJUMP1(pmi_errno != UPMI_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmi_kvs_commit", "**pmi_kvs_commit %d", pmi_errno);

        pmi_errno = UPMI_BARRIER();
        MPIR_ERR_CHKANDJUMP1(pmi_errno != UPMI_SUCCESS, mpi_errno, MPI_ERR_OTHER, "**pmi_barrier", "**pmi_barrier %d", pmi_errno);
    }
